...
public class DutchNationalFlag {
	...
	public static void dutchFlagPartition(int pivotIndex, List<Color> A) {
		...
		List<Color> lesserThanPivot = new ArrayList<>();
		List<Color> equalsThanPivot = new ArrayList<>();
		List<Color> greaterThanPivot = new ArrayList<>();

		for (Color color : A) {
			int current = color.ordinal();
			if (current < pivotValue)
				lesserThanPivot.add(color);
			else if (current == pivotValue)
				equalsThanPivot.add(color);
			else
				greaterThanPivot.add(color);
		}

		for (int i = 0; i < lesserThanPivot.size(); i++) {
			A.set(i, lesserThanPivot.get(i));
		}

		int start = lesserThanPivot.size();
		for (int i = 0; i < equalsThanPivot.size(); i++) {
			A.set(start++, equalsThanPivot.get(i));
		}

		for (int i = 0; i < greaterThanPivot.size(); i++) {
			A.set(start++, greaterThanPivot.get(i));
		}
		return;
	}
